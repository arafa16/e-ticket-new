...
const UserTicket = () => {
    ...
    //get clear ticket__________________________________________________________//

    useEffect(()=>{
        getNewTickets();
        getMe();
    },[user, limitNew, pageNew])
            ...
            <div className='flex items-end justify-end'>
                <div className={`${!ticket ? 'bg-white' : ''}  px-2 flex justify-center py-1`}>
                    <button className={` px-2 text-[10pt] hover:bg-cyan-500 hover:text-white`} onClick={()=>handleForm()}>
                        {!ticket ? 
                            '+ create ticket' 
                            : <BiXCircle className='text-[24px] text-white animate-bounce hover:animate-pulse ' />}
                    </button>
                </div>
            </div>
        </div>
        <div className={`w-full px-4 pb-6 ${!ticket ? 'hidden' : ''}`}>
            <FormTicket user={user} newTicket={getNewTickets} clearTicket={getClearTickets} setTicket={setTicket} />
        </div>
